...
def send_email(msg):
    # SMTP object for sending email
    smtp_obj = smtplib.SMTP(SMTP_HOST, SMTP_TLS_PORT)

    # Identify to ESMTP server using EHLO
    smtp_obj.ehlo()

    # Put the SMTP connection in TLS mode.
    smtp_obj.starttls()

    # Login using custom created email account
    # Password X'ed out
    smtp_obj.login('CSC122StockScrapeSender@gmail.com', 'XXXXXXXXXXXX')

    # Send the email to second custom created email account
    smtp_obj.sendmail('CSC122StockScrapeSender@gmail.com', 'CSC122StockScrapeReceiver@gmail.com', 'Subject: StockWatcher update \n' + msg)
